...
use crate::proto_gen::NdJsonReadOptionsProto;
use crate::schema::decode_optional_schema;
use crate::{bytes, proto_compression, NativeResult};

pub(crate) fn json_options(
    options_ptr: *const u8,
    options_len: usize,
    schema_ptr: *const u8,
    schema_len: usize,
) -> NativeResult<JsonReadOptions<'static>> {
    let p = NdJsonReadOptionsProto::decode(bytes(options_ptr, options_len)?)?;
