...
namespace Apache.DataFusion.TableProviders.Sqlite.Sql;

internal sealed class SqlQueryBuilder(SqlDialect dialect)
{
    public PushedQuery Build(string baseQuery, StreamingTableScanRequest request)
    {
        ...
        SqlSelectStatement statement = new(
        ...
        List<SqlQueryParameter> parameters)
    {
        ...
        for (int i = 0; i < request.Filters.Count; i++)
        {
            ...
            parameters.Add(new SqlQueryParameter(parameterName, ConvertFilterValue(filter)));
        }
