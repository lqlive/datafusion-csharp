...
internal sealed class ClickHouseArrowArrayStream : IArrowArrayStream
{
    ...
    private readonly IReadOnlyList<ClickHouseQueryParameter>? parameters;
        ...
        IReadOnlyList<ClickHouseQueryParameter>? parameters = null)
    {
        ...
        // batch is pulled. The schema is already known from registration, so a
        // consumer that only inspects the schema never pays for a round trip,
        // and the query cost shifts out of plan establishment into the first
        // read.
        this.schema = schema;
