...
/// <summary>
/// Builds a MySQL connection pool once and reuses it to register any number of
/// tables into one or more <see cref="SessionContext"/> instances. Prefer this
/// over the one-shot <see cref="SessionContext.RegisterMySql(string, MySqlTableOptions, CancellationToken)"/>
...
/// </summary>
public sealed class MySqlTableFactory : IDisposable
{
    ...
    /// </summary>
    public MySqlTableFactory(string connectionString, CancellationToken cancellationToken = default)
    {
        ...
        handle = NativeCancellationBridge.Invoke(
            (ulong token, out IntPtr created) => NativeMethods.df_mysql_table_factory_new(nativeConnectionString.Pointer, token, out created),
            cancellationToken);
    }
    ...
    private IntPtr Handle
    {
        get
        {
            ...
            {
                throw new ObjectDisposedException(nameof(MySqlTableFactory));
            }
    ...
    public void Register(SessionContext context, string registrationName, string tableName, string? schemaName = null, CancellationToken cancellationToken = default)
    {
        ...
        NativeCancellationBridge.Invoke(
            token => NativeMethods.df_mysql_table_factory_register(factoryHandle, contextHandle, nativeName.Pointer, nativeSchema?.Pointer ?? IntPtr.Zero, nativeTable.Pointer, token),
            cancellationToken);
    }
    ...
    public void Dispose()
    {
        ...
        handle = IntPtr.Zero;
        NativeMethods.ThrowIfError(NativeMethods.df_mysql_table_factory_free(current));
    }
}
